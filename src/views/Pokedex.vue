  ...
  <Layout>
    <div class="flex items-center justify-center lg:justify-end">
      <div class="home-switch w-fit flex justify-around items-center space-x-2">
        <button class="rounded-full bg-gray-100 px-4 py-2 flex items-center text-sm font-medium">
          <div
            @click="activeTab = 'keep'"
            :class="activeTab === 'keep' ? 'home-switch-active cursor-pointer py-1 px-4 text-white bg-yellow-400 rounded-full' : 'cursor-pointer py-1 px-4 text-black'"
          >
            Keep
          </div>
          <div
            @click="activeTab = 'history'"
            :class="activeTab === 'history' ? 'home-switch-active cursor-pointer py-1 px-4 text-white bg-yellow-400 rounded-full' : 'cursor-pointer py-1 px-4 text-black'"
          >
            History
          </div>
        </button>
...
onMounted(() => {
  ...
  if (keepData) {
    ...
    console.log("pokemon.Keepdata:", pokemonKeep.value);
  }
